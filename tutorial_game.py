...
def isInside(sprite, mouse_x, mouse_y) -> bool:
    # gets the sprites bounding box
    bounds = sprite.getWorldBounds()

    # checks if the mouse falls within the bounding box
...
class MyASGEGame(pyasge.ASGEGame):
    ...
    def keyHandler(self, event: pyasge.KeyEvent) -> None:
        ...
        if event.action == pyasge.KEYS.KEY_PRESSED and self.menu:

            # use right/left or A/D keys to select play/exit options
            if event.key == pyasge.KEYS.KEY_RIGHT or event.key == pyasge.KEYS.KEY_LEFT or \
                    event.key == pyasge.KEYS.KEY_D or event.key == pyasge.KEYS.KEY_A:
            ...
            # if the enter or Z key is pressed, action the menu option
            if event.key == pyasge.KEYS.KEY_ENTER or event.key == pyasge.KEYS.KEY_Z:
                if self.menu_option == 0:
                    self.menu = False
                else:
                    self.signalExit()
            ...
            """
            allows you to spawn fish with the S key (for testing purposes)
